...
- **App is partially implemented**: The full pipeline (file detection, transcription, LLM classification, and Notes routing/creation) is wired end-to-end in `AppDelegate` on macOS 26+. The menu bar icon (`MenuBarExtra`) is implemented and shows a static placeholder popover. Routing to Reminders and Calendar remains unimplemented

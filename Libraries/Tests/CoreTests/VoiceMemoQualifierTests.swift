...
struct VoiceMemoQualifierTests {
    ...
    func rejectsExactlyThreshold() {
        let url = URL(fileURLWithPath: "/tmp/recording.m4a")
        let result = qualifier.qualifies(url: url, fileSize: 1024)
        #expect(result == nil)
    }
    ...
    func rejectsBelowThreshold() {
        let url = URL(fileURLWithPath: "/tmp/recording.m4a")
        let result = qualifier.qualifies(url: url, fileSize: 512)
        #expect(result == nil)
    }
    ...
    func rejectsZeroBytes() {
        let url = URL(fileURLWithPath: "/tmp/recording.m4a")
        let result = qualifier.qualifies(url: url, fileSize: 0)
        #expect(result == nil)
    }
    ...
    func qualifiesJustAboveThreshold() {
        let url = URL(fileURLWithPath: "/tmp/recording.m4a")
        let result = qualifier.qualifies(url: url, fileSize: 1025)

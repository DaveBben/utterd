...
1. [x] Create `Utterd/Features/MenuBar/MenuBarPopoverView.swift`: a SwiftUI `View` struct containing a `VStack(alignment: .leading, spacing: 8)` with: `Text(MenuBarStrings.title)` using `.font(.headline)`, `Text(MenuBarStrings.subtitle)` using `.font(.subheadline).foregroundStyle(.secondary)`, `Divider()`, `Button(MenuBarStrings.settingsButton) { }` (no-op), and `Button(MenuBarStrings.quitButton) { NSApplication.shared.terminate(nil) }`. Apply `.padding()` to the VStack and set `.frame(minWidth: 220)`.
2. [x] Modify `Utterd/App/UtterdApp.swift`: remove the `WindowGroup` scene and its `.defaultSize` and `.commands { AppCommands() }` modifiers. Add a conditional `MenuBarExtra` that only appears when `appState.permissionResolved` is `true`: use `if appState.permissionResolved { MenuBarExtra("Utterd", systemImage: "waveform") { MenuBarPopoverView() }.menuBarExtraStyle(.window) }`. Keep the `Settings` scene with `.environment(appState)`. Keep `@NSApplicationDelegateAdaptor` and `@State private var appState`. Wire `appDelegate.appState = appState` in an `.onAppear` or initializer so the delegate can set the flag.
3. [x] Run `xcodegen generate && xcodebuild -scheme Utterd -destination 'platform=macOS' build` to verify compilation
4. [x] Run `xcodebuild -scheme Utterd -destination 'platform=macOS' test` to verify the full test suite passes (no regressions from WindowGroup removal)

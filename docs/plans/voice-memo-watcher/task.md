...
- **Seen-set data structure: `emittedPaths: Set<URL>` with permanent emission tracking** — Tracks files for which an event has already been emitted. Only files that qualify (pass `VoiceMemoQualifier`) are added to the set. This naturally handles the mid-sync scenario (AC-02.3): files that are too small at startup are not added to the set, so when FSEvents re-reports them after they grow past the 1024-byte threshold, they are re-evaluated and emitted. Once emitted, the URL remains in the set permanently — subsequent size changes for that path do NOT trigger re-emission (AC-01.5 requires at most one event per file path). The set is cleared when the watched directory disappears, so files re-created after recovery are detected. Alternative considered: `[URL: Int64?]` dictionary mapping URLs to sizes — rejected as unnecessary complexity since the simpler `Set<URL>` correctly handles all scenarios.
